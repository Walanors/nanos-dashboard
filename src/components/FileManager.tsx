...
export default function FileManager() {
  ...
  // Base directory paths based on the current tab
  const getBasePath = () => {
        ...
        <table className="w-full font-mono text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              files.map((file) => (
                <tr key={file.path} className="border-b border-amber-500/10 hover:bg-amber-500/5">
                  <td className="py-2">
                    {file.isDirectory ? (
                      <button
                        type="button"
                        onClick={() => handleDirectoryChange(file.path)}
                        className="flex items-center text-amber-300 hover:underline"
                      >
